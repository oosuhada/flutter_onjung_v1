...
import 'package:flutter_onjung_v1/v2/v2_glass.dart';
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [
          BoxShadow(
              color: Color(0x22C5522C), blurRadius: 24, offset: Offset(0, 12)),
        ],
      ),
      child: Column(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(label,
              style: const TextStyle(color: Color(0xFFFFE9E0), fontSize: 12)),
...
class _RecordsScreenState extends ConsumerState<RecordsScreen> {
  ...
  Widget build(BuildContext context) {
    final records = ref.watch(onjungProvider).records.where((r) {
      ...
      return r.person.toLowerCase().contains(query) ||
          r.relation.toLowerCase().contains(query) ||
          r.event.toLowerCase().contains(query) ||
            ...
            child: SafeArea(
              ...
              child: ListView(
                ...
                children: [
                  if (record.assetPath != null)
                    ClipRRect(
                      ...
                      child: SizedBox(
                        height: 180,
                        child:
                            Image.asset(record.assetPath!, fit: BoxFit.cover),
                      ),
                    ),
                  ...
                  const _TimelineRow(
                      title: '이번 기록',
                      detail: '관계와 상황을 함께 저장했어요',
                      emphasized: true),
                  const _TimelineRow(
                      title: '지난 기록', detail: '이 관계의 이전 온정 기록을 이어서 확인할 수 있어요'),
                ],
              ),
            ),
          ),
...
class _TimelineRow extends StatelessWidget {
  const _TimelineRow(
      {required this.title, required this.detail, this.emphasized = false});
  final String title;
  final String detail;
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
                ...
                color: emphasized ? _brand : Colors.black26),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
...
class _AddRecordScreenState extends ConsumerState<AddRecordScreen> {
  ...
  final _noteController =
      TextEditingController(text: '결혼식 참석 예정. 식사 여부는 아직 미정.');
  bool _isSent = true;
  String _event = '결혼식';
  int _amount = 100000;
  ...
  void _save() {
    ...
    if (person.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('상대 이름을 입력해 주세요.')));
      return;
    }
                ...
                children: [
                  ...
                  Align(
                    ...
                    child: AppGlassSegmentedControl<bool>(
                      ...
                      selected: _isSent,
                      labelBuilder: (value) => value ? '보냈어요' : '받았어요',
                      onSelected: (value) => setState(() => _isSent = value),
                    ),
                  ),
                  const SizedBox(height: 22),
                  ...
                  TextField(
                    ...
                    maxLines: 3,
                    decoration:
                        const InputDecoration(hintText: '참석 여부, 선물, 기억할 내용'),
                  ),
                  const SizedBox(height: 28),
                  FilledButton(
    ...
    return SafeArea(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          ...state.records.where((r) => r.date.month == 8).take(3).map((r) =>
              Padding(
                  padding: const EdgeInsets.only(bottom: 10),
...
class MyOnjungScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final sent =
        records.where((r) => r.isSent).fold<int>(0, (sum, r) => sum + r.amount);
    ...
    return SafeArea(
      ...
      child: ListView(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                  child: _SummaryCard(
                      label: '보낸 온정',
                      value: '${_won.format(sent)}원',
                      icon: Icons.north_east_rounded)),
              const SizedBox(width: 10),
              Expanded(
                  child: _SummaryCard(
                      label: '받은 온정',
                      value: '${_won.format(received)}원',
          ...
          const SizedBox(height: 12),
          const _PersonInsight(
              name: '김서연',
              relation: '대학 친구',
              count: '최근 3년 4회',
              balance: '보낸 25만 · 받은 20만'),
          const SizedBox(height: 10),
          const _PersonInsight(
              name: '박준호',
              relation: '회사 동료',
              count: '최근 2년 3회',
              balance: '보낸 20만 · 받은 10만'),
          const SizedBox(height: 26),
        ...
        children: [
          ...
          const Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('친구 관계의 온정이 가장 활발해요',
                    style:
                        TextStyle(fontWeight: FontWeight.w900, fontSize: 16)),
                SizedBox(height: 6),
                Text('최근 기록의 46%가 친구 관계예요. 결혼식 기록이 가장 많고 평균 금액은 10만원이에요.',
